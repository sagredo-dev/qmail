...
static int oneread(ssize_t (*op)(int, const void *, size_t), int fd, char *buf, int len)
{
  int r;
...
int substdio_feed(substdio *s)
{
  ...
  if (s->p) return s->p;
  q = s->n;
  r = oneread(s->op,s->fd,(char *)s->x,q);
  if (r <= 0) return r;
  s->p = r;
  q -= r;
  s->n = q;
  if (q > 0) /* damn, gotta shift */ byte_copyr((char *)s->x + q,r,s->x);
  return r;
}
